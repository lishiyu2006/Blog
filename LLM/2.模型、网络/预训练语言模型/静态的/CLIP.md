...
- 假设一批训练数据包含 N 个图文对，将每个文本通过文本编码器得到文本特征向量 $t_i$​，每个图像通过图像编码器得到图像特征向量 $T_i$​（i=1,2,⋯,N）。
- 计算文本特征向量和图像特征向量之间的相似度，通常使用点积运算，即相似度分数 $s_{ij}​=I_iT T_j​/||ti​|| ||vj||)$，其中 sij​ 表示第 i 个文本和第 j 个图像之间的相似度。
- 对于每个文本，其正样本是与之对应的图像，负样本是其他所有图像；对于每个图像，其正样本是与之对应的文本，负样本是其他所有文本。
- 定义对比学习损失函数（如 InfoNCE 损失）：
    
    $L=−N1​∑i=1N​[log∑j=1N​exp(s_ij​/τ)exp(s_ii​/τ)​+log∑j=1N​exp(s_ji​/τ)exp(s_ii​/τ)​]$
    
    其中，τ 是温度超参数，用于调整相似度分数的分布，通过最小化这个损失函数，使得匹配的图文对的相似度分数 sii​ 尽可能大，不匹配的图文对的相似度分数 sij​（i=j）尽可能小
