...
### 微观：
首先推出 $x_t$ 和 $x_{t-1}$ 的关系是 $x_{t-1}$ 乘上权重 $\sqrt{α_t}$ 加上噪声 $ε_{t-1}$ 乘上噪声权重 $\sqrt{1 - α_t}$ ,得到 $x_t$： 
$x_t = \sqrt{α_t} * x_{t-1} + \sqrt{1 - α_t} * ε_{t-1}$                                                                             （1）
因为噪声符合正态分布，并且知道和 $x_{t-1} = sqrt(α_{t-1}) * x_{t-2} + sqrt(1 - α_{t-1}) * z_2$ ， 将  将 $x_{t-1}$ 的表达式**代入**到 $x_t$ 的表达式中，得到：$x_{t-1}$ 的表达式**代入**到 $x_t$ 的表达式中，得到：
...
3. $\sqrt{\bar{α}_t}$ (图像权重)
    ...
    - **怎么得到**： 
        ...
        - 先设定一个微小的“每步噪声强度” $\beta_t$（如论文中0.0001~0.0002），$\beta_t$ 可固定或随步数轻微增大。
        - 计算关系：$\alpha_t = 1 - \beta_t$，再通过累积得到 $\bar{\alpha}_t = \prod_{i=1}^{t} \alpha_i$ 。
        - 其中 $\beta_t$ 怎么算：
