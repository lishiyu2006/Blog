...
#### 参数:
"loky"（默认）：多进程，CPU 密集型（计算多）、数据处理（如 numpy/pandas 运算）	自动规避 GIL 锁，支持内存共享（大数据高效）
"threading"	： 多线程，IO 密集型（等待多，如文件读写、网络请求），切换开销低，共享进程内存（无需 IPC 通信）
"multiprocessing"	：多进程，兼容旧版本场景，基于 Python 内置 multiprocessing，功能不如 loky 完善