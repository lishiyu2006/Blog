...
`
说明：值的顺序比较灵活，但又遵循一定的习惯，`background-size`一定要跟在`background-position`后面，用/分隔
...
说明：严格的，`font-size`和`font-family`是必须的，`line-height`必须跟在`font-height`后面，用/分隔

语法：`[style]` `[variant]` `[weight]` `[line-height]` `[family]`
